...
RASCO in short. First, section~\ref{s:pf} described profile framework that this work based. Then, section~\ref{s:ss} describes a new redundancy abstraction called Strips to track and handle redundant WuObjects dynamically. Lastly, section~\ref{s:dfd}, ~\ref{s:fr} and ~\ref{s:reconfig} presents the distributed algorithm used to solve the problem. %#############The models and algorithms are tested extensively on various benchmarks, described in section~\ref{s:benchmarks}, and the results are discussed and compared with existing fault tolerant system models in section~\ref{s:results}##############
